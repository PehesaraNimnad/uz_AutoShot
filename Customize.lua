...
-- When enabled, every generated thumbnail is automatically uploaded to your
-- FiveManage team straight after it's captured, and the resulting CDN URL is
-- written into shots/cdn-map.json. illenium-appearance's web UI reads that
-- small manifest and loads images from the CDN instead of from this
-- resource, so players only ever download a few KB of JSON instead of the
-- whole shots/ folder full of PNGs.
-- 1) Create a free account at https://fivemanage.com
