...
// ────────────────────────────────────────────────────────────────
// FiveManage CDN upload
// After a thumbnail is saved to disk, optionally push it straight to
// FiveManage and record the resulting CDN URL in shots/cdn-map.json.
// illenium-appearance's web UI fetches that manifest and prefers the CDN
// URL over the local NUI path, so players stop downloading every PNG in
// this resource's files{} bundle.
// ────────────────────────────────────────────────────────────────

const FM_ENABLED      = GetConvar('uz_autoshot_fm_enabled', 'false') === 'true';
const FM_API_KEY      = GetConvar('uz_autoshot_fm_apikey', '');
const FM_FOLDER       = GetConvar('uz_autoshot_fm_folder', 'clothing');
const FM_DELETE_LOCAL = GetConvar('uz_autoshot_fm_deletelocal', 'true') === 'true';
const FM_API_URL      = 'https://api.fivemanage.com/api/v3/file';
const FM_CONCURRENCY  = 4;

const CDN_MAP_PATH = path.join(OUTPUT_DIR, 'cdn-map.json');

let cdnMap = {};
try {
    if (fs.existsSync(CDN_MAP_PATH)) {
        cdnMap = JSON.parse(fs.readFileSync(CDN_MAP_PATH, 'utf8')) || {};
    }
} catch (err) {
    console.log('^3[uz_AutoShot]^0 Could not read existing cdn-map.json, starting fresh: ' + err.message);
    cdnMap = {};
}

let cdnMapDirty = false;
let cdnMapSaveTimer = null;
function scheduleSaveCdnMap() {
    cdnMapDirty = true;
    if (cdnMapSaveTimer) return;
    cdnMapSaveTimer = setTimeout(() => {
        cdnMapSaveTimer = null;
        if (!cdnMapDirty) return;
        cdnMapDirty = false;
        try {
            fs.writeFileSync(CDN_MAP_PATH, JSON.stringify(cdnMap));
        } catch (err) {
            console.log('^1[uz_AutoShot]^0 Failed to save cdn-map.json: ' + err.message);
        }
    }, 1500);
}

let fmWarnedRuntime = false;
let fmActiveUploads = 0;
const fmQueue = [];

function queueFiveManageUpload(key, buffer, ext) {
    if (!FM_ENABLED) return;
    fmQueue.push({ key, buffer, ext });
    pumpFiveManageQueue();
}

function pumpFiveManageQueue() {
    while (fmActiveUploads < FM_CONCURRENCY && fmQueue.length > 0) {
        const job = fmQueue.shift();
        fmActiveUploads++;
        uploadToFiveManage(job.key, job.buffer, job.ext).finally(() => {
            fmActiveUploads--;
            pumpFiveManageQueue();
        });
    }
...
async function uploadToFiveManage(key, buffer, ext) {
    if (!FM_API_KEY) {
        console.log('^1[uz_AutoShot]^0 FiveManage upload is enabled but Customize.FiveManage.ApiKey is empty.');
        return;
    }
    if (typeof fetch !== 'function' || typeof FormData !== 'function' || typeof Blob !== 'function') {
        if (!fmWarnedRuntime) {
            fmWarnedRuntime = true;
            console.log('^1[uz_AutoShot]^0 FiveManage upload needs fetch/FormData/Blob support in the server Node runtime. Update your FXServer artifact to enable this.');
        }
        return;
    }

    try {
        const safeName = key.replace(/[\\/]/g, '_') + '.' + ext;
        const form = new FormData();
        form.append('file', new Blob([buffer]), safeName);
        form.append('filename', safeName);
        if (FM_FOLDER) form.append('path', FM_FOLDER);

        const res = await fetch(FM_API_URL, {
            method: 'POST',
            headers: { Authorization: FM_API_KEY },
            body: form,
        });

        let json = null;
        try { json = await res.json(); } catch (_) { /* ignore parse errors, handled below */ }

        if (!res.ok || !json || !json.data || !json.data.url) {
            console.log('^1[uz_AutoShot]^0 FiveManage upload failed for ' + key + ' (HTTP ' + res.status + ')' + (json ? ': ' + JSON.stringify(json) : ''));
            return;
        }

        cdnMap[key] = json.data.url;
        scheduleSaveCdnMap();
        console.log('^2[uz_AutoShot]^0 CDN uploaded: ' + key + ' -> ' + json.data.url);

        if (FM_DELETE_LOCAL) {
            const localPath = path.join(OUTPUT_DIR, key + '.' + ext);
            fs.unlink(localPath, () => {});
        }
    } catch (err) {
        console.log('^1[uz_AutoShot]^0 FiveManage upload error for ' + key + ': ' + (err && err.message ? err.message : err));
    }
}

// Catch up on anything shot before CDN upload was enabled, or that failed
// to upload last time (e.g. server restarted mid-batch). Runs once on start.
function scanExistingShotsForUpload() {
    if (!FM_ENABLED) return;
    (function walk(dir) {
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            const rel = path.relative(OUTPUT_DIR, full).split(path.sep).join('/');
            const ext = path.extname(entry.name).slice(1);
            const key = rel.slice(0, rel.length - ext.length - 1);
            if (cdnMap[key]) continue;

            try {
                queueFiveManageUpload(key, fs.readFileSync(full), ext);
            } catch (err) {
                console.log('^3[uz_AutoShot]^0 Skipped backlog upload for ' + key + ': ' + err.message);
            }
        }
    })(OUTPUT_DIR);
}
...
if (FM_ENABLED) {
    setImmediate(scanExistingShotsForUpload);
}
...
onNet('uz_autoshot:server:processCapture', (payload) => {
    ...
    try {
        ...
        queueFiveManageUpload(xFilename, outputData, ext);
    } catch (err) {
